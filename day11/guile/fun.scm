...
(define adjacent-count    
  (lambda (grid x y)
    (+ (if (occupied? grid (- x 1) (- y 1)) 1 0)
       (if (occupied? grid (- x 1) y) 1 0)
       (if (occupied? grid (- x 1) (+ y 1)) 1 0)
       (if (occupied? grid x (- y 1)) 1 0)
       ;;(if (occupied? grid x y) 1 0)
       (if (occupied? grid x (+ y 1)) 1 0)
       (if (occupied? grid (+ x 1) (- y 1)) 1 0)
       (if (occupied? grid (+ x 1) y) 1 0)
       (if (occupied? grid (+ x 1) (+ y 1)) 1 0))))
         ...
         (#t 
          (cond
           ...
           ((and (empty-seat? grid x y)
            ...
            (grid-set! grid2 x y #\#))
           ;; seat occupied and 4 or more seats occupied .. seat becomes empty
           ((and (occupied? grid x y)
                 (>= (adjacent-count grid x y) 4))
...
> (solution)
...
occupied seat count 2164  ........ ACCEPTED
|#
